...
---
### 0.5 连接验证结果（2026-07-06 实测）

**三级验证全部通过**，凭证 `REDACTED_SAP_COMM_USER` 有效，无需解锁。

| 级别 | 状态 | 结果 |
|------|------|------|
| 第 1 级 网络层 | ✅ | 租户可达，返回 401（需要认证） |
| 第 2 级 认证层 | ✅ | Basic Auth 成功，返回 200 |
| 第 3 级 API 层 | ✅ | 12/15 端点 OK，3 个 403（需开通 Arrangement） |

**15 端点完整探测结果**（`Integration/probe-2026-07-06.json`）：

| 场景 | 端点 | 通信场景 | 状态 |
|------|------|---------|------|
| INT-15 | 物料主数据 V2 | SAP_COM_0009 | ✅ 200 |
| INT-15 | 物料主数据 V4 | SAP_COM_0009 | ✅ 200 |
| INT-13 | 供应商 V2 | SAP_COM_0008 | ✅ 200 |
| INT-13 | 供应商公司 V2 | SAP_COM_0008 | ✅ 200 |
| INT-02 | 采购订单 V2 | legacy | ✅ 200 |
| INT-02 | 采购订单 V4 | SAP_COM_0053 | ✅ 200 |
| INT-02 | 采购订单行 V4 | SAP_COM_0053 | ✅ 200 |
| INT-02 | 采购计划行 V4 | SAP_COM_0053 | ✅ 200 |
| INT-07 | 物料凭证 V2 | SAP_COM_0108 | ✅ 200 |
| INT-09 | 供应商发票 V2 | legacy | ✅ 200 |
| INT-09 | 供应商发票行 V2 | legacy | ✅ 200 |
| INT-01 | 采购预测（计划订单） | — | ✅ 200（**新发现，可用！**） |
| INT-05 | 要货计划（计划协议） | SAP_COM_0103 | ❌ 403（需开通） |
| INT-08 | 质检结果（检验批） | — | ❌ 403（需开通） |
| INT-12 | 付款状态（日记账） | — | ❌ 403（需开通） |

**关键发现**：
- `REDACTED_SAP_COMM_USER` 凭证有效，密码可用，**无需解锁**
- INT-01 采购预测的 `API_PLANNED_ORDERS` 实测可用（之前标注"未探测"）
- 3 个 403 端点（INT-05/08/12）需在 Fiori 开通对应 Communication Arrangement
- 出站写权限（INT-03/06/11 的 PATCH/POST）仍需单独验证
