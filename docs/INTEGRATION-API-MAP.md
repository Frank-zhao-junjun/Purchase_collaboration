...
### 7.3 出站 OData 写操作确认（公有云，非 RFC）

| 场景 | 函数模块 | 事务码 | 官方状态 |
|------|---------|--------|---------|
| INT-03 | OData V4 PATCH `api_purchaseorder_2/PurchaseOrder` | SAP_COM_0053 | 公有云不适用 BAPI_PO_CHANGE（本地部署方案），改用 OData PATCH，需 Arrangement 开通 UPDATE |
| INT-11 | OData V2 POST `A_SupplierInvoice` | legacy V2 | 公有云不适用 BAPI_INCOMINGINVOICE_CREATE（本地部署方案），改用 OData POST，需 Arrangement 开通 CREATE |

> **公有云不适用 RFC/BAPI**（本地部署方案）。出站写操作改用 OData PATCH/POST，需 SAP Basis 团队在 Communication Arrangement 中开通对应通信场景的 UPDATE/CREATE 权限。